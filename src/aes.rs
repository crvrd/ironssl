...
fn mix_cols(data: &mut [u8; 16]) {
    ...
    for i in 0..16 {
        let row = i % 4;
        data[i] = match row {
            0 => MUL2[tmp[i]] ^ MUL3[tmp[i+1]] ^ tmp[i+2] as u8           ^ tmp[i+3] as u8,
            1 => tmp[i-1] as u8         ^ MUL2[tmp[i]]   ^ MUL3[tmp[i+1]] ^ tmp[i+2] as u8,
            2 => tmp[i-2] as u8         ^ tmp[i-1] as u8           ^ MUL2[tmp[i]]   ^ MUL3[tmp[i+1]],
            3 => MUL3[tmp[i-3]] ^ tmp[i-2] as u8         ^ tmp[i-1] as u8           ^ MUL2[tmp[i]],
...
fn inv_mix_cols(data: &mut [u8; 16]) {
    ...
    for i in 0..16 {
        let row = i % 4;
        data[i] = match row {
            0 => MUL14[tmp[i]]   ^ MUL11[tmp[i+1]] ^ MUL13[tmp[i+2]] ^ MUL9[tmp[i+3]],
            1 => MUL9[tmp[i-1]]  ^ MUL14[tmp[i]]   ^ MUL11[tmp[i+1]] ^ MUL13[tmp[i+2]],
            2 => MUL13[tmp[i-2]] ^ MUL9[tmp[i-1]]  ^ MUL14[tmp[i]]   ^ MUL11[tmp[i+1]],
            3 => MUL11[tmp[i-3]] ^ MUL13[tmp[i-2]] ^ MUL9[tmp[i-1]]  ^ MUL14[tmp[i]],
...
fn schedule_keys(key: & [u8; 16]) -> [[u8; 16]; 11] {
    ...
    for i in 1..keys {
        ...
        roundkeys[i][0] = SBOX[t[1] as usize] ^ RCON[i-1];
        roundkeys[i][1] = SBOX[t[2] as usize];
        roundkeys[i][2] = SBOX[t[3] as usize];
        roundkeys[i][3] = SBOX[t[0] as usize];
